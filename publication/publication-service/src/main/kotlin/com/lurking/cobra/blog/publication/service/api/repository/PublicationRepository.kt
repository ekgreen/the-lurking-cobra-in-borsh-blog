...
interface PublicationRepository : MongoRepository<PublicationEntity, String> {
    ...
    @Query("db.publication.find({'status': ${Status.PUBLICATION_READY}, 'last_publication': null}).sort({'rating':-1}).limit(?0)")
    ...
    @Query("db.publication.find({'status': ${Status.PUBLICATION_READY}, 'last_publication': {\$lte:new Date(ISODate().getTime() - 1000 * 60 * 60 * 24 * 30),\$gt:new Date(ISODate().getTime() - 1000 * 60 * 60 * 24 * 180)}}).sort({'rating':-1, 'count_publication':1}).limit(?0)")
